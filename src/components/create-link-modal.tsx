...
export function CreateLinkModal({ open, onOpenChange, onSuccess }: CreateLinkModalProps) {
    ...
    const [previewTab, setPreviewTab] = useState<"default" | "x" | "linkedin" | "facebook">("default");
    ...
    const shortLink = domain && slug ? `https://${domain}/${slug}` : domain ? `https://${domain}/...` : "https://...";
            ...
            >
                ...
                <div className="flex items-center justify-between px-5 py-3.5 border-b border-border/50">
                    <div className="flex items-center gap-2 min-w-0">
                        <span className="text-sm font-medium text-foreground truncate">Links</span>
                    ...
                    <div className="p-5 space-y-4 min-w-0">
                        ...
                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between">
                                <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Short Link</Label>
                                <div className="flex items-center gap-0.5">
                                    <Button type="button" variant="ghost" size="icon" className="h-7 w-7 rounded-md" title="Clear">
                                        <X className="h-3.5 w-3.5 text-muted-foreground" />
                                    </Button>
                                    <Button type="button" variant="ghost" size="icon" className="h-7 w-7 rounded-md" title="Generate">
                            ...
                            <div className="flex rounded-lg border overflow-hidden focus-within:ring-2 focus-within:ring-ring/50">
                                <Select value={domain} onValueChange={(v) => setDomain(v ?? "")}>
                                    <SelectTrigger className="w-[130px] rounded-none border-0 border-r bg-muted/40 h-10 text-sm">
                        ...
                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between">
                                ...
                                <Button type="button" variant="ghost" size="sm" className="h-6 text-xs text-muted-foreground px-2">
                                    Manage
                                </Button>
                            </div>
                            <div className="relative">
                        ...
                        <div className="flex items-center justify-between py-1">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">Track conversions for this link</p>
                                </div>
                            </div>
                                ...
                                >
                                    {activeSection === "utm" && (
                                        <div className="space-y-3">
                                            ...
                                            <Input placeholder="Title" value={ogTitle} onChange={(e) => setOgTitle(e.target.value)} className="rounded-lg h-9 text-sm" />
                                            <Textarea placeholder="Description" value={ogDescription} onChange={(e) => setOgDescription(e.target.value)} className="rounded-lg min-h-[50px] text-sm" />
                                            <Input type="url" placeholder="Image URL" value={ogImageUrl} onChange={(e) => setOgImageUrl(e.target.value)} className="rounded-lg h-9 text-sm" />
                                        </div>
                                    )}
                    ...
                    <div className="border-l border-border/50 bg-muted/30 p-5 space-y-5 hidden md:block">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Select value={folderId} onValueChange={(v) => setFolderId(v ?? "")}>
                                <SelectTrigger className="rounded-lg h-9 bg-background text-sm">
                        ...
                        {/* QR Code */}
                        <div className="space-y-1.5">
                            <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">QR Code</Label>
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Link Preview</Label>
                            </div>
                                ...
                                <div className="flex border-b">
                                    {PREVIEW_TABS.map((tab) => (
                                        <button
                                            ...
                                            className={`flex items-center gap-1 px-2.5 py-1.5 text-xs transition-colors ${previewTab === tab.id
