...
export function CreateLinkModal({ open, onOpenChange, onSuccess }: CreateLinkModalProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={(v) => { if (!v) resetForm(); onOpenChange(v); }}>
            <DialogContent showCloseButton={false} className="w-full max-w-[calc(100%-2rem)] sm:max-w-none md:max-w-4xl max-h-[95vh] overflow-y-auto overflow-x-hidden p-0 gap-0 border-none shadow-2xl">
                {/* Header */}
